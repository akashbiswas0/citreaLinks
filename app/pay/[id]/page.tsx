...
export default function PayDetailPage() {
  ...
  // Use wagmi hooks for transaction - using useSendTransaction for native cBTC transfer
  ...
  const onPay = async () => {
    ...
    try {
      ...
      // Send native cBTC transaction using sendTransaction
      sendTransaction({
        to: to as `0x${string}`,
  ...
  const fallback = (
    `<div style="display:flex;align-items:center;justify-content:center;min-height:100vh;background:linear-gradient(135deg,#0f0f23,#1a1a2e,#16213e);color:white;">
      <div style="background:rgba(255,255,255,0.05);backdrop-filter:blur(20px);padding:32px 36px;border-radius:24px;max-width:600px;width:94%;border:1px solid rgba(255,255,255,0.1);box-shadow:0 25px 50px rgba(0,0,0,0.3);">
        ...
        <div style="opacity:0.9;font-size:18px;text-align:center;margin-bottom:8px;">💳 ${amount} cBTC → ${to}</div>
        <div style="margin-top:20px;text-align:center;opacity:0.7;font-size:12px;">🔒 Secure Payment with Citrea Network</div>
        <div style="margin-top:10px;text-align:center;opacity:0.6;font-size:11px;">ℹ️ Memo functionality not supported on Citrea EVM</div>
      </div>
    </div>`
  );
  ...
  <div className="relative bg-gradient-to-br from-green-900/40 to-emerald-900/30 border border-green-400/40 rounded-2xl p-6 backdrop-blur-xl shadow-2xl">
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <span className="text-sm">View on Citrea Explorer</span>
      </div>
